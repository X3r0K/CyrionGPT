...
/**
 * Processes message content for normal mode, converting storage IDs to signed URLs
 */
function processNormalModeContentWithUrls(
  ...
  const processedContent = message.content.map((item) => {
    if (item.type === 'image_url') {
      const signedUrl = imageUrlMap.get(item.image_url.url);
      return signedUrl
...
/**
 * Processes message content for terminal mode, reorganizing images and text with signed URLs
 */
function processTerminalModeContentWithUrls(
  message: BuiltChatMessage,
  imageUrlMap: Map<string, string>,
): BuiltChatMessage {
  ...
  // Convert storage IDs to signed URLs and separate content types
  message.content.forEach((item) => {
    if (item.type === 'image_url') {
...
export async function processMessagesWithImagesUnified(
  ...
  // Convert storage IDs to signed URLs (single batch call)
  const imageUrlMap = await getImageUrls(Array.from(storageIds));

  // Process messages with signed URL substitution
  const processedMessages = messagesWithProcessedAssistantImages.map(
    (message) =>
      isTerminalMode
        ? processTerminalModeContentWithUrls(message, imageUrlMap)
        : processNormalModeContentWithUrls(message, imageUrlMap),
  );
